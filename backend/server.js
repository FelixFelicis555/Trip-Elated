...
return res.status(400).send({
    message:`Adventure not found for ${req.query.adventure}!`,
});
});

// Make new reservation

// If the reservation is successful,it flips available key to false and reserved key to true

// Expects serialized form data 
...
app.post("/reservation/new",(req,res)=>{
    ...
    if(reqDate>currentDate){
        db.get("detail").find((item)=>item.id==req.body.adventure).assign({reserved:true,available: false}).write();
       ...
       reservation.name=reservation.name.trim().toLowerCase().split(" ").map((i,j)=>i.charAt(0).toUpperCase()+i.slice(1)).join(" ");
